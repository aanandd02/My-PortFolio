  ...
  return (
    <>
      <button
        ...
        className="fixed bottom-6 left-5 z-[70] flex items-center gap-3 rounded-full border border-cyan-300/35 bg-slate-950/90 px-3 py-2 text-left text-cyan-100 shadow-[0_14px_40px_rgba(8,145,178,0.35)] backdrop-blur-xl transition hover:-translate-y-0.5 hover:bg-slate-900"
      >
        <span className="flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br from-cyan-300 via-sky-400 to-emerald-300 text-sm font-bold text-slate-950 shadow-[0_0_25px_rgba(34,211,238,0.35)]">
          AI
        </span>
        <span className="pr-1">
      ...
      <div
        aria-hidden={!isOpen}
        className={`fixed bottom-20 left-5 z-[70] w-[95vw] max-w-[460px] overflow-hidden rounded-[28px] border border-cyan-300/20 bg-[radial-gradient(circle_at_top,_rgba(34,211,238,0.14),_transparent_42%),linear-gradient(180deg,rgba(2,6,23,0.98),rgba(2,6,23,0.95))] shadow-[0_0_3rem_-0.5rem_rgba(6,182,212,0.65)] transition-all duration-300 ease-out ${
          isOpen
      ...
      >
        <div className="border-b border-white/10 px-4 pb-4 pt-4">
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-start gap-3">
              <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-gradient-to-br from-cyan-300 via-sky-400 to-emerald-300 font-bold text-slate-950">
                AI
              </div>
              <div>
                <p className="text-base font-semibold text-white">Anand AI Assistant</p>
                <div className="mt-1 flex items-center gap-2 text-xs text-cyan-100/75">
            ...
            <button
              type="button"
              onClick={resetChat}
              className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-[11px] font-medium text-gray-300 transition hover:bg-white/10 hover:text-white"
            >
              New chat
            </button>
          </div>
        ...
        <div
          ref={listRef}
          className="max-h-[27rem] space-y-3 overflow-y-auto overscroll-contain px-3 py-4"
        >
          {messages.map((msg, index) => (
            ...
            >
              <div
                className={`max-w-[85%] rounded-2xl px-3.5 py-3 text-sm transition-all duration-200 ${
                  msg.role === "user"
        ...
        <div className="border-t border-white/10 bg-slate-950/70 p-3">
          <div className="mb-3 flex items-center justify-between gap-2">
            <div className="flex flex-wrap gap-2">
              {STARTER_QUESTIONS.map((question) => (
                <button
                  ...
                  className="rounded-full border border-cyan-300/20 bg-cyan-300/10 px-3 py-1.5 text-[11px] text-cyan-100 transition hover:border-cyan-300/35 hover:bg-cyan-300/20 disabled:cursor-not-allowed disabled:opacity-60"
                  disabled={isThinking}
                >
                  {question}
                </button>
            ...
            {resumeUrl ? (
              <a
                ...
                className="text-[11px] font-medium text-cyan-200 transition hover:text-cyan-100"
              >
                Open Resume
              </a>
            ) : null}
          ...
          >
            ...
            <button
              type="submit"
              className="rounded-xl bg-gradient-to-r from-cyan-300 to-emerald-300 px-4 py-3 text-sm font-semibold text-slate-950 transition hover:brightness-105 disabled:cursor-not-allowed disabled:opacity-60"
